...
class NetworkResponse {
  ...
  NetworkResponse(
      {required this.statusCode, required this.data, this.success = false, this.error});
  ...
  static NetworkResponse valError(String data, int i) {
    return NetworkResponse(data: data, success: false, statusCode: i);
  }
  ...
  static NetworkResponse netError(e) {
    return NetworkResponse(
        data: json.encode({
          'message': 'It seems like we could not find an internet connection.'
        }),
        success: false,
        statusCode: 600,
        error: e);
...
class NetworkRequestMaker {
  ...
  Future<NetworkResponse> execute(
    ...
    try {
      request = http.Request(method??'GET', finalUrl);
    } catch (e) {
      print(e);
    ...
    try {
      response = await _client
          .send(request)
          .timeout(Duration(seconds: NetworkRequestMaker.timeoutSeconds??10));
    } catch (e) {
      print(e);
      return NetworkResponse.netError(e);
    }
